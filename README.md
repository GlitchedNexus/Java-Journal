...
This project is a desktop journaling and life-organization app that brings multiple planning and tracking tools into a single workflow.

### Conditions

Must have at least four functionalities, of which one must include an arbitrary number of inputs into a list.
...
People who are looking for an integrated organisation system that combines all necessary tools for life organisation and future planning.

### Why is this project of interest to you?

I am very interested in optimising my life to achieve maximum productivity, so having an app that allows me to integrate all my favourite features into one workflow is very appealing.

## Tech Stack

- **Language:** Java
- **GUI framework:** Java Swing (JFrame, JPanel, JOptionPane)
- **Persistence:** File-based save/load of user data
- **Code style:** Checkstyle with Google Java Style configuration (see `checkstyle.xml`)
- **IDE / project setup:** IntelliJ IDEA project (`Project-Starter.iml`)
- **Design artefacts:** UML design diagram (`UML_Design_Diagram.png`)

### User Stories

1. As a user, I want to be able to change my habit goals to suit my needs.
...
5. As a user, I want to be able to track my achievements for the day.
6. As a user, I want to be able to customise the journal prompt if I feel like I want to change it.
7. As a user, I want to be able to sign in and out of my account.
...
10. As a user, I want to be able to load the work I have saved if I choose to.

### Instructions for Grader

The app features buttons for each task the app can handle.
Buttons are labelled to indicate the function they perform, including the
load and save buttons. Additionally, you will receive a `JOptionPane` confirmation message when
data is loaded. All "add X to Y" options will display a `JOptionPane` asking for input. Furthermore,
you will also have a button to show you all items and to manipulate the number of items (for example, remove an item or clear all).
